...
SELECT department_name, round(avg(salary)) as salary FROM employees
    JOIN departments USING(department_id)
    GROUP BY department_name ORDER BY salary;

SELECT department_name, count(employee_id) as "Number of Employees" FROM employees
    JOIN departments USING(department_id)
...
SELECT extract(YEAR FROM hire_date) as hire_year, count(*) as "Number of Employees" FROM employees
    GROUP BY hire_year;
